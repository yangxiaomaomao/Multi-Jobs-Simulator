...
class Cluster():
    ...
    def init_gandiva(self): 
        gandiva_list = [self.gandiva_1, self.gandiva_2, self.gandiva_4, self.gandiva_8]
        ...
        machine_list = {
            ...
            4:self.machine_ids[lengths[0] + lengths[1]:lengths[0] + lengths[1] + lengths[2]],
            8:self.machine_ids[lengths[0] + lengths[1] + lengths[2]:]
        }
        return machine_list
    ...
    def get_candidate_place(self, node_info, label_counter, job:Job):
        candidate_list = list()

        job_worker_num = job.worker_num
        print(job.job_id, job.arrive_ts, job.worker_num)
        #sys.exit(0)
        ...
        # sys.exit(0)
        #candidate_list[0] = ['G89', 'G88', 'G90', 'G91']
        #print(candidate_list,"llll")
        candidate_list = [sorted(candi, key=lambda x: int(x[1:])) for candi in candidate_list]
        
        job_parallel = job.parallel
        if job.is_vision_job():
            return candidate_list
        else:
            pp, tp = job_parallel["pp"], job_parallel["tp"]
            if pp == 1:
                return candidate_list
            elif job.worker_num == 4:
                candidate_list = [list(perm) for sublist in candidate_list for perm in permutations(sublist)]
                return candidate_list
            elif job.worker_num == 8:
                return candidate_list
