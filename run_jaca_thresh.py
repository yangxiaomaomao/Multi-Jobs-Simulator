...
jaca_thresh_list = [1.8,1.6,1.4,1.2,1.0,0.8,0.6]
replace = int(sys.argv[1])
if not replace:
    all_trace_result_dir = "result_jaca_thresh"
    baseline_list = [
        "aef-te",
        "slf-te",
        "spf-te",
    ...
    all_trace_result_dir = "result_jaca_thresh_replace"
    baseline_list = [
        "jaca-load_balance",
        "jaca-gandiva",
        "jaca-consolidate",
...
def run(jaca_thresh, baseline):
    if not replace:
        sched, placer = parse_sched(baseline)
    else:
        sched, placer = baseline.split("-")[0], baseline.split("-")[1]
    thresh_dir = "%s/thresh-%2.1f" % (all_trace_result_dir, jaca_thresh)
    result_dir = "%s/%s-%s" % (thresh_dir, sched, placer)
    ...
    os.system("cp %s %s" % (trace_file, thresh_dir))
    os.system("cp plot.ipynb %s" % thresh_dir)
...
if __name__ == "__main__":
    process_list = []
    start_time = time.time()
    for jaca_thresh in jaca_thresh_list:
        for baseline in baseline_list:
            p = multiprocessing.Process(target=run, args=(jaca_thresh, baseline))
