...
if not replace:
    all_trace_result_dir = "result_nic_cap"
    baseline_list = [
        "aef-te",
        "slf-te",
        "spf-te",
    ...
    all_trace_result_dir = "result_nic_cap_replace"
    baseline_list = [
        "jaca-load_balance",
        "jaca-gandiva",
        "jaca-consolidate",
...
nic_cap_list = np.array([25,50,75,100,125,150,200]) / 8 * 1000 # 5-10-15-20-25 # 10 is finish in the early exp
...
if __name__ == "__main__":
    process_list = []
    start_time = time.time()
    for nic_cap in nic_cap_list:
        for baseline in baseline_list:
            p = multiprocessing.Process(target=run, args=(nic_cap, baseline))
