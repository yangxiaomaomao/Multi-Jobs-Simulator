...
all_trace_result_dir = "result_comm_level"
# jaca param
...
# mid has finish in other exp
comm_level_list = ["low", "high"]
...
def run(comm_level, baseline):
    sched, placer = parse_sched(baseline)
    level_dir = "%s/comm_level-%s" % (all_trace_result_dir, comm_level)
    result_dir = "%s/%s-%s" % (level_dir, sched, placer)
    # the trace file containing the arrive time and the model spec(comm pattern, iter num and so on)
    trace_file = "trace/comm_%s_trace.json" % comm_level
    
    os.makedirs(result_dir, exist_ok=True)
    os.system("rm -f %s/*.txt" % result_dir)
    os.system("cp %s %s" % (trace_file, level_dir))
    os.system("cp plot.ipynb %s" % level_dir)
...
if __name__ == "__main__":
    process_list = []
    start_time = time.time()
    for comm_level in comm_level_list:
        for baseline in baseline_list:
            p = multiprocessing.Process(target=run, args=(comm_level, baseline))
